...
<head>
    ...
    <style>
        ...
        /* Animasi Transisi Halaman (Cepat) */
    ...
    <script>
        localStorage.setItem('brain_kpi_total_docs', '{{ number_format($kpiData['total_documents']) }}');
        localStorage.setItem('brain_kpi_project_docs', '{{ number_format($kpiData['document_project']) }}');
        localStorage.setItem('brain_kpi_fungsi_docs', '{{ number_format($kpiData['document_fungsi']) }}');
        localStorage.setItem('brain_kpi_active_users', '{{ number_format($kpiData['active_users_30d']) }}');
        localStorage.setItem('brain_top_project', 'RDMP RU VI Balongan Phase I');
    </script>
</head>
<body class="text-slate-800 flex h-screen overflow-hidden animate-fade-in">
    ...
    <aside class="w-56 bg-[#F5F7FA] border-r border-gray-200 flex flex-col py-4 px-3 flex-shrink-0 transition-all">
        ...
        <nav class="space-y-1">
            ...
            <a href="{{ route('chat.ai') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-gray-600 hover:bg-gray-100 font-semibold transition text-sm">
            ...
            <div class="space-y-6 max-w-[1600px] mx-auto pb-10">
                ...
                <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-200 mb-4 flex flex-col gap-3">
                    ...
                    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                        ...
                        <form action="{{ route('dashboard') }}" method="GET" id="filterForm" class="flex flex-wrap sm:flex-nowrap items-center gap-3 w-full lg:w-auto relative">
                            <div class="relative w-full sm:w-56 lg:w-64 mt-2 sm:mt-0">
                                ...
                                <div class="relative">
                                    <select name="project" onchange="submitFilter()" class="w-full bg-white border border-gray-300 hover:border-blue-400 text-gray-800 text-xs font-bold rounded-lg focus:ring-0 focus:border-blue-600 block pl-3 pr-8 py-2 appearance-none cursor-pointer transition-all outline-none shadow-sm">
                                    ...
                                    <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3 text-gray-500"><svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7" /></svg></div>
                                </div>
                            </div>
                            <div class="relative w-full sm:w-36 mt-2 sm:mt-0">
                                ...
                                <div class="relative">
                                    <select name="year" onchange="submitFilter()" class="w-full bg-white border border-gray-300 hover:border-blue-400 text-gray-800 text-xs font-bold rounded-lg focus:ring-0 focus:border-blue-600 block pl-3 pr-8 py-2 appearance-none cursor-pointer transition-all outline-none shadow-sm">
                            ...
                            <div class="relative w-full sm:w-36 mt-2 sm:mt-0">
                                ...
                                <div class="relative">
                                    <select name="month" onchange="submitFilter()" class="w-full bg-white border border-gray-300 hover:border-blue-400 text-gray-800 text-xs font-bold rounded-lg focus:ring-0 focus:border-blue-600 block pl-3 pr-8 py-2 appearance-none cursor-pointer transition-all outline-none shadow-sm">
                            ...
                            <div id="filter-loader" class="hidden absolute -top-8 right-0 flex items-center gap-2 text-blue-600 font-bold text-xs bg-blue-50 px-2 py-1 rounded shadow-sm border border-blue-200">
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-left text-xs text-gray-600">
                                ...
                                <tbody id="tbody-recent" class="qa-tbody">
                                    ...
                                    <tr class="bg-white border-b border-gray-100 hover:bg-blue-50/50 transition-colors cursor-pointer group" onclick="window.location='{{ route('document.preview', ['doc' => $file['name']]) }}'">
                                        <td class="py-3 pl-4"><input type="checkbox" class="rounded border-gray-300 text-blue-600 focus:ring-blue-500 w-3.5 h-3.5 cursor-pointer" onclick="event.stopPropagation()"></td>
        ...
        <div id="activityModalContent" class="bg-white w-full max-w-4xl rounded-2xl shadow-2xl flex flex-col transform scale-95 transition-transform duration-300 max-h-[85vh]">
            ...
            <div class="p-6 overflow-y-auto custom-scrollbar flex-grow">
                <table class="w-full text-left">
                    ...
                    <tbody class="text-xs" id="modalAuditTableBody"></tbody>
                </table>
            </div>
            <div class="px-6 py-4 border-t border-gray-200 bg-gray-50/80 rounded-b-2xl text-right">
                 <button onclick="closeActivityModal()" class="text-sm font-bold text-gray-700 hover:text-gray-900 bg-white border-2 border-gray-300 px-6 py-2 rounded-xl shadow-sm hover:bg-gray-100 transition focus:outline-none">Close Window</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function generateRandomLog() {
            ...
            return {
                ...
                document: t.doc, location: t.loc, time: 'Just now', icon: a.icon
            };
        }
        ...
        function triggerNewLiveLog() {
            ...
            liveTr.innerHTML = `
                <td class="py-2.5 pl-3 flex items-center gap-3">
                    <div class="w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-black border-2 border-white shadow-md ${log.avatar_color}">${log.initial}</div>
                ...
                <td class="py-2.5">
                    <span class="px-2.5 py-1 rounded-lg text-[9px] font-bold border flex items-center gap-1.5 w-fit shadow-sm ${log.action_color}">
                        <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">${log.icon}</svg>${log.action_label}
                    </span>
                </td>
                <td class="py-2.5">
            ...
            if (modalTbody.children.length < 25) {
                ...
                modalTr.innerHTML = `
                    <td class="py-3 pl-3 flex items-center gap-3">
                        <div class="w-9 h-9 rounded-full flex items-center justify-center text-xs font-black border-2 border-white shadow-md ${log.avatar_color}">${log.initial}</div>
                    ...
                    <td class="py-3">
                        <span class="px-3 py-1.5 rounded-lg text-[10px] font-bold border flex items-center gap-1.5 w-fit shadow-sm ${log.action_color}">
                            <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">${log.icon}</svg>${log.action_label}
                        </span>
                    </td>
                    <td class="py-3">
                    ...
                    <td class="py-3 text-right pr-3"><span class="text-xs text-gray-500 font-semibold">${log.time}</span></td>
                `;
        ...
        setInterval(() => {
            ...
            document.getElementById('live-online-counter').innerText = currentOnline;
        }, 3500); 

        const trendingPool = [
            '#HAZOP_Balongan', '#Kontrak_EPC_Tuban', '#P&ID_Cilacap', '#Progress_Report_RU_V', 
            '#Variation_Order_03', '#Vendor_Audit_2026', '#HSE_Incident_Log', '#Budget_Approval_Q1'
        ];
        ...
        function renderWatchlist() {
            ...
            watchlist.forEach(proj => {
                ...
                const baseClass = isSelected ? 'bg-blue-50 border-blue-300 text-blue-700 shadow-sm' : 'bg-white border-gray-300 text-gray-700 hover:border-gray-400 hover:shadow-sm';
                const pill = document.createElement('div');
                pill.className = `flex items-stretch border text-[10px] font-bold rounded-lg transition-all ${baseClass}`;
                pill.innerHTML = `
                    <a href="?project=${encodeURIComponent(proj)}" class="px-3 py-1.5 flex items-center hover:text-blue-700 rounded-l-lg cursor-pointer truncate max-w-[150px]" title="${proj}">${proj}</a>
                    <div class="w-px bg-gray-200 my-1.5"></div>
                    <button onclick="removePin('${proj}')" class="px-2 hover:bg-red-50 hover:text-red-600 rounded-r-lg text-gray-400 flex items-center justify-center transition-colors" title="Remove Pin"><svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M6 18L18 6M6 6l12 12" /></svg></button>
                `;
        ...
        function openPinModal() { const m = document.getElementById('pinModal'), c = document.getElementById('pinModalContent'); m.classList.remove('hidden'); void m.offsetWidth; m.classList.remove('opacity-0'); c.classList.remove('scale-95'); }
        function closePinModal() { const m = document.getElementById('pinModal'), c = document.getElementById('pinModalContent'); m.classList.add('opacity-0'); c.classList.add('scale-95'); setTimeout(() => m.classList.add('hidden'), 300); }
        function addPin() { const v = document.getElementById('newPinSelect').value; if (v && !watchlist.includes(v)) { watchlist.push(v); localStorage.setItem('brain_watchlist', JSON.stringify(watchlist)); renderWatchlist(); } closePinModal(); }
        function removePin(proj) { watchlist = watchlist.filter(p => p !== proj); localStorage.setItem('brain_watchlist', JSON.stringify(watchlist)); renderWatchlist(); }

        function openActivityModal() { const m = document.getElementById('activityModal'), c = document.getElementById('activityModalContent'); m.classList.remove('hidden'); void m.offsetWidth; m.classList.remove('opacity-0'); c.classList.remove('scale-95'); }
        function closeActivityModal() { const m = document.getElementById('activityModal'), c = document.getElementById('activityModalContent'); m.classList.add('opacity-0'); c.classList.add('scale-95'); setTimeout(() => { m.classList.add('hidden'); }, 300); }
        ...
        function switchQaTab(tabName) {
            document.querySelectorAll('.qa-tab').forEach(el => { el.classList.remove('bg-[#145D40]', 'text-white', 'shadow-sm'); el.classList.add('bg-gray-50', 'text-gray-500', 'border-gray-200'); });
            const activeTab = document.getElementById('tab-' + tabName);
            activeTab.classList.remove('bg-gray-50', 'text-gray-500', 'border-gray-200'); activeTab.classList.add('bg-[#145D40]', 'text-white', 'shadow-sm');
        ...
        function toggleQuickAccess() {
            const c = document.getElementById('quickAccessContent'), t = document.getElementById('qaToggleText'), i = document.getElementById('qaToggleIcon');
            if (c.classList.contains('hidden')) { c.classList.remove('hidden'); t.innerText = "Hide table"; i.classList.remove('rotate-180'); } 
        ...
        let projectChartInstance = null; let trendChartInstance = null; let isProjectExpanded = false; let isTrendExpanded = false;
        ...
        const getProjectOptions = (names, values, colors) => {
            return {
                ...
                plotOptions: { bar: { borderRadius: 4, horizontal: true, barHeight: '65%', distributed: true, dataLabels: { position: 'top' } } },
                colors: colors,
                dataLabels: { enabled: true, textAnchor: 'start', offsetX: 20, style: { fontSize: '13px', colors: ['#1e293b'], fontWeight: 900, fontFamily: 'Inter, sans-serif' }, formatter: function (val) { return val.toLocaleString('id-ID'); }, dropShadow: { enabled: false } },
                xaxis: { categories: names, labels: { show: true, formatter: val => (val >= 1000000 ? (val/1000000).toFixed(1)+'m' : (val/1000).toFixed(0)+'k'), style: { fontSize: '11px', colors: '#64748b', fontWeight: 600 } }, axisBorder: { show: false }, axisTicks: { show: false } },
        ...
        const getTrendOptions = (seriesData, colors) => {
            return {
                series: seriesData,
                chart: { type: 'area', height: '490', stacked: false, toolbar: { show: false }, fontFamily: 'Inter, sans-serif', zoom: { enabled: false } },
                colors: colors,
                stroke: { curve: 'smooth', width: 3.5 }, 
                fill: { type: 'gradient', gradient: { shadeIntensity: 1, opacityFrom: 0.4, opacityTo: 0.0, stops: [0, 100] } }, 
                dataLabels: { enabled: false },
                xaxis: { type: 'category', categories: chartCategories, labels: { style: { fontSize: '11px', colors: '#64748b', fontWeight: 600 } }, axisBorder: { show: false }, axisTicks: { show: false }, tooltip: { enabled: false } },
                yaxis: { labels: { formatter: val => (val >= 1000 ? (val/1000).toFixed(0)+'k' : val), style: { fontSize: '11px', colors: '#64748b', fontWeight: 600 } } },
                legend: { position: 'bottom', horizontalAlign: 'left', fontSize: '11px', fontWeight: 800, markers: { radius: 6, width: 12, height: 12 }, itemMargin: { horizontal: 8, vertical: 4 }, offsetY: 5, onItemHover: { highlightDataSeries: true } },
                ...
                tooltip: { theme: 'light', shared: true, intersect: false, fixed: { enabled: false }, custom: function({series, seriesIndex, dataPointIndex, w}) { if (dataPointIndex < 0 || dataPointIndex >= tooltipDates.length) return null; let fullDateTitle = tooltipDates[dataPointIndex]; let totalVal = 0; w.config.series.forEach((s, i) => { if(s.data && s.data[dataPointIndex] > 0) totalVal += s.data[dataPointIndex]; }); let dailyData = []; w.config.series.forEach((s, i) => { if(s.data && s.data[dataPointIndex] > 0) dailyData.push({ name: s.name, val: s.data[dataPointIndex], color: w.config.colors[i] }); }); dailyData.sort((a, b) => b.val - a.val); let tooltipList = ""; dailyData.forEach(item => { tooltipList += `<div class="flex justify-between items-center mb-1.5"><div class="flex items-center gap-2 overflow-hidden w-[240px]"><span class="w-2.5 h-2.5 rounded-sm shadow-sm flex-shrink-0" style="background-color: ${item.color}"></span><span class="text-gray-700 text-[11px] font-bold truncate leading-tight" title="${item.name}">${item.name}</span></div><span class="font-black text-gray-900 text-[13px]">${item.val.toLocaleString('id-ID')}</span></div>`; }); return `<div class="bg-white/95 backdrop-blur-md shadow-2xl rounded-xl p-4 min-w-[400px] border border-gray-100"><div class="mb-3 pb-2">${tooltipList}</div><div class="border-t border-gray-200 pt-3 mt-1"><div class="flex justify-between items-end"><div><div class="text-[10px] text-gray-500 uppercase tracking-widest mb-1 font-bold">Timeline Point</div><div class="text-sm font-black text-blue-600 flex items-center gap-1.5"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg> ${fullDateTitle}</div></div><div class="text-right"><div class="text-[10px] text-gray-500 uppercase tracking-widest mb-1 font-bold">Aggregated Upload</div><div class="text-2xl font-black text-gray-900">${totalVal.toLocaleString('id-ID')}</div></div></div></div></div>`; } }
            };
        };

        function toggleProjectChart() { isProjectExpanded = !isProjectExpanded; const btn = document.getElementById('btnProject'); if (isProjectExpanded) { projectChartInstance.updateOptions(getProjectOptions(fullProjectNames, fullProjectValues, fullProjectColors)); btn.innerText = "Show Top 10"; } else { projectChartInstance.updateOptions(getProjectOptions(topProjectNames, topProjectValues, topProjectColors)); btn.innerText = "View All Projects"; } }
        function toggleTrendChart() { isTrendExpanded = !isTrendExpanded; const btn = document.getElementById('btnTrend'); if (isTrendExpanded) { trendChartInstance.updateOptions({ series: fullWaveSeries, colors: fullWaveColors }); btn.innerText = "Show Top 5"; } else { trendChartInstance.updateOptions({ series: waveSeries, colors: waveColors }); btn.innerText = "View All Trends"; } }

        projectChartInstance = new ApexCharts(document.querySelector("#chartProject"), getProjectOptions(topProjectNames, topProjectValues, topProjectColors)); projectChartInstance.render();
        trendChartInstance = new ApexCharts(document.querySelector("#chartTrend"), getTrendOptions(waveSeries, waveColors)); trendChartInstance.render();
    </script>
</body>
</html>