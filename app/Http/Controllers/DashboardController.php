...
class DashboardController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // =========================================================================
        // 6. DYNAMIC DRILL-DOWN DATA (Discipline Breakdown)
        // =========================================================================
        $disciplineData = [];
        $disciplines = ['Process', 'Mechanical', 'Piping', 'Civil', 'Electrical', 'Instrument', 'HSE'];
        $dispColors = ['#008FFB', '#00E396', '#FEB019', '#FF4560', '#775DD0', '#3F51B5', '#03A9F4'];

        foreach ($barChartData as $proj) {
            $total = $proj['total'];
            $projDiscipline = [];
            
            // Logika Bobot agar jumlah disiplin PAS = Total, dan TIDAK ADA MINUS
            $weights = [];
            $totalWeight = 0;
            ...
            $remaining = $total;
            foreach ($disciplines as $idx => $disp) {
                if ($idx === count($disciplines) - 1) {
                    $val = $remaining; // Sisa ditambahkan ke elemen terakhir
                } else {
                    $val = (int) floor($total * ($weights[$idx] / $totalWeight));
                    $remaining -= $val;
                }
                $projDiscipline[] = ['name' => $disp, 'val' => $val, 'color' => $dispColors[$idx]];
            }
            // Urutkan dari terbesar ke terkecil
            usort($projDiscipline, function($a, $b) { return $b['val'] <=> $a['val']; });
            $disciplineData[$proj['name']] = $projDiscipline;
        }
       ...
       $kpiData = [
            ...
            'document_fungsi' => 196355,
            'total_users' => 1240,
            'active_users_30d' => 202
        ];

        $lifecycleData = [
           ['phase' => '01. Initiation', 'count' => floor($grandTotalDocuments * 0.02), 'active' => true, 'icon' => 'M4 6h16M4 12h16M4 18h7'],
            ['phase' => '02. Pre-FS', 'count' => floor($grandTotalDocuments * 0.03), 'active' => true, 'icon' => 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z'],
            ['phase' => '03. Pre-FID/Early Work', 'count' => floor($grandTotalDocuments * 0.01), 'active' => true, 'icon' => 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4'],
            ...
            ['phase' => '05. FEED', 'count' => floor($grandTotalDocuments * 0.14), 'active' => false, 'icon' => 'M19.428 15.428a2 2 0 00-1.022-.547l-2.384-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z'],
            ['phase' => '06. FS & FID', 'count' => floor($grandTotalDocuments * 0.01), 'active' => false, 'icon' => 'M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z'],
            ['phase' => '07. EPC Bidding', 'count' => floor($grandTotalDocuments * 0.03), 'active' => false, 'icon' => 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z'],
            ['phase' => '08. EPC Work', 'count' => floor($grandTotalDocuments * 0.44), 'active' => false, 'icon' => 'M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4'],
            ['phase' => '09. Operation', 'count' => floor($grandTotalDocuments * 0.015), 'active' => false, 'icon' => 'M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4'],
            ['phase' => '10. Closing', 'count' => floor($grandTotalDocuments * 0.005), 'active' => false, 'icon' => 'M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4'],
        ];
        ...
        return view('brain', compact(
            'kpiData', 'projectsData', 'disciplineData',
